...
class RecuperarDadosAcesso(BaseModel):
    chave_pix: str
    telefone: str

@router.post("/recuperar-dados-acesso")
@limiter.limit("3/minute")
async def recuperar_dados_acesso(request: Request, dados: RecuperarDadosAcesso, db: Session = Depends(get_db)):
    """Recupera CPF e E-mail (mascarados) através da Chave Pix e Telefone."""
    # Normalizar telefone (remover parênteses, espaços, traços)
    telefone_limpo = re.sub(r'[^0-9]', '', dados.telefone)
    
    # Busca por combinação única de Chave Pix E Telefone
    # O telefone no DB pode estar em formatos diferentes, então fazemos uma busca mais flexível se necessário,
    # mas por padrão comparamos o que foi enviado (limpo) com o valor no banco.
    
    # Nota: Em um cenário real, poderíamos precisar de tratamento de DDI ou filtros LIKE.
    # Aqui usaremos igualdade direta após limpeza básica se possível.
    ...
    # Filtrar pelo telefone manualmente para lidar com variações de formatação no DB
    usuario_encontrado = None
    for u in usuarios:
        u_tel_limpo = re.sub(r'[^0-9]', '', u.telefone) if u.telefone else ""
        if u_tel_limpo == telefone_limpo:
            usuario_encontrado = u
            break
            
    if not usuario_encontrado:
        # Segurança: Não informar se o erro foi Pix ou Telefone
        import time
        time.sleep(0.5)
        raise HTTPException(
            status_code=404, 
            detail="Nenhum usuário encontrado com os dados informados. Verifique se digitou os dados corretamente."
        )
    
    return {
        "cpf_mascarado": mascarar_cpf(usuario_encontrado.cpf),
        "email_mascarado": mascarar_email(usuario_encontrado.email),
        "mensagem": "Dados localizados com sucesso. Use-os para redefinir sua senha se necessário."
    }
