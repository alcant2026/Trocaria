...
async def obter_snapshot_dashboard(db: Session = Depends(get_db), usuario: Usuario = Depends(obter_usuario_logado)):
    """
    Endpoint consolidado (Snapshot) que retorna todos os dados necessários 
    para o dashboard do usuário em uma única requisição.
    Reduz o número de chamadas HTTP e sobrecarga no servidor.
    """
    from modelos.modelos_db import Transacao, TipoTransacao, SolicitacaoEmprestimo, Investimento, GarantiaSocial, AcessoInvestidor, StatusSolicitacao
    from sqlalchemy import func
    from datetime import timezone, timedelta
    
    # Configuração de Fuso (Reaproveitado)
    ...
    # 2. Histórico de Transações (Comum a todos)
    ...
    # 3. Dados Específicos por Perfil
    if usuario.is_admin:
        # Lógica de Admin (Pendências e Fiscal Simplificado)
        from rotas.rotas_financeiro import obter_resumo_fiscal
        from rotas.rotas_financeiro import listar_pendentes
    ...
    # Perfil Tomador
    # Meus Empréstimos e Garantias Pendentes
    ...
    # Perfil Investidor
    # Solicitações Disponíveis e Minha Carteira
